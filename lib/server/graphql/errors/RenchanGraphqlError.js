...
export default class RenchanGraphqlError extends Error {
   ...
   * @throws {Error} - 101.X000.001 {"memberName":"RenchanGraphqlError.get:errorCode"}get:errorCode]
   */
  static get errorCode () {
     ...
     */
    throw new Error('101.X000.001 {"memberName":"RenchanGraphqlError.get:errorCode"}')
  }
  ...
  /**
   * Convert into a GraphQL error.
   *
   * `graphql` collects errors as `GraphQLError`,
   * and only that class carries the members a transport serializes an error with.
   * An error handed to one of them is converted here,
   * and stays reachable as its `originalError`.
   *
   * @returns {GraphQLError} - GraphQL error wrapping this error.
   * @public
   */
  toGraphQLError () {
