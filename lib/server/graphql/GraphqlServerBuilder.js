...
export default class GraphqlServerBuilder {
   ...
   *
   * @returns {typeof GraphqlServerBuilder} - Constructor.
   */
  get Ctor () {
   ...
   *
   * @returns {typeof validate} - validate function.
   */
  static get validate () {
    return validate
  }
   ...
   * get: specifiedRules.
   *
   * @returns {typeof specifiedRules} - Validation rules the GraphQL specification defines.
   */
  static get specifiedRules () {
  ...
  buildHttpServer () {
    ...
    const mountedApp = this.mountRouteToApp({
      routes: [
        ...
        /*
         * NOTE:
         * When implementing the same endpoint for both GraphQL and GraphiQL (= Graph + i + QL),
         * the GraphQL HTTP route must be registered after the GraphiQL route in the routing configuration.
         */
        graphqlHttpRoute,
      ],
    })
  ...
  setupServer ({
    ...
    const validateHandler = this.defineValidateHandler()
  ...
  /**
   * Define validate handler.
   *
   * `graphql-ws` calls `validate` with the schema and the document alone, so
   * composing `specifiedRules` with the rules of this endpoint is done here.
   *
   * @returns {(
   *   schema: GraphqlType.Schema,
   *   document: import('graphql').DocumentNode,
   * ) => ReadonlyArray<import('graphql').GraphQLError>} - Validate handler.
   */
  defineValidateHandler () {
    const endpointRules = this.graphqlHandlerBuilder.validationRules
      ?? []

    const rules = [
      ...this.Ctor.specifiedRules,
      ...endpointRules,
    ]
