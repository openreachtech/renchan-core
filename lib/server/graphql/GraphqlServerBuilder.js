...
export default class GraphqlServerBuilder {
  ...
  /**
   * NOTE: require paths
   * GraphQL の middleware を生成するクラスを <GraphqlMiddlewareFactory> とする
   * - resolvers: actual, stub # CustomerResolverHashFactory.get:actualResolversDirectoryPath
   *     ResolverHashFactory に path を渡せば、 <GraphqlMiddlewareFactory> に集約できる
   * - schema: 'customer.graphql' # CustomerGraphqlHttpMiddlewareFactory#createSchema()
   *     現時点で <GraphqlMiddlewareFactory>
   * - routePath: '/graphql-customer' # AppServer#composeRoutes()
   *     <GraphqlMiddlewareFactory> が ServerRoute なら、routePath は ServerRoute に持たせる
   *
   * <GraphqlMiddlewareFactory> は、以下の static method を持つ
   * - .get:actualResolversPath
   * - .get:stubResolversPath
   * - .get:schemaFilePath
   */
