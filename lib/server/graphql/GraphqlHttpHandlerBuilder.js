...
export default class GraphqlHttpHandlerBuilder {
  ...
  static async createAsync ({
    ...
    const validationRules = await this.buildValidationRules({
      engine,
    })
   ...
   *
   * `graphql-http` composes `specifiedRules` itself, so what is handed over is
   * the rules of this endpoint alone.
   *
   * @param {{
   *   engine: GraphqlType.ServerEngine
   * }} params - Parameters of this factory method.
   * @returns {Promise<Array<GraphqlType.ValidationRule>>} - Validation rules.
   */
  static async buildValidationRules ({
    engine,
  }) {
    const extraRules = this.extraCreateHandlerParams.validationRules
      ?? []

    const validators = await engine.collectRequestValidators()

    const rules = validators.map(it =>
      it.defineValidationRule()
    )
   ...
   * get: createHandler.
   *
   * @returns {typeof createHandler} - createHandler function.
   */
  static get createHandler () {
   ...
   *
   * @returns {typeof GraphqlHttpHandlerBuilder} - Constructor.
   */
  get Ctor () {
 ...
 *   validationRules?: Array<GraphqlType.ValidationRule>
