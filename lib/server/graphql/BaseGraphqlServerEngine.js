...
import IntrospectionAccessedGraphqlRequestValidator from './validators/concretes/IntrospectionAccessedGraphqlRequestValidator.js'
import DocumentTooDeepGraphqlRequestValidator from './validators/concretes/DocumentTooDeepGraphqlRequestValidator.js'
...
export default class BaseGraphqlServerEngine {
   ...
   *   IntrospectionAccessed: string
   *   DocumentTooDeep: string
   *   Database: string
  ...
  /**
   * Collect the request validator classes of GraphQL.
   *
   * What is listed here is the candidates rather than the decision,
   * and the order is the order the validators are applied in.
   *
   * @returns {Array<GraphqlType.RequestValidatorCtor>} - Array of request validator classes.
   */
  static collectGraphqlRequestValidatorCtors () {
    return [
   ...
   *
   * @returns {typeof BaseGraphqlServerEngine} - Constructor.
   */
  get Ctor () {
   ...
   * When want to change the settings of Express application, override this method.
   *
  ...
  /**
   * Collect request validators.
   *
   * @returns {Promise<Array<GraphqlType.RequestValidator>>} - Array of request validators.
   * @public
   */
  async collectRequestValidators () {
    return this.buildGraphqlRequestValidators()
  }

  /**
   * Build the request validators of GraphQL.
   *
   * Each validator declares whether it applies to an endpoint,
   * so the candidates are narrowed here rather than where they are listed.
   *
   * @returns {Promise<Array<GraphqlType.RequestValidator>>} - Array of request validators.
   */
  async buildGraphqlRequestValidators () {
    const ValidatorCtors = this.Ctor.collectGraphqlRequestValidatorCtors()

    const acceptableCtors = ValidatorCtors.filter(it =>
      it.isAcceptable({
        engine: this,
      })
    )

    const validatorPromises = acceptableCtors.map(it =>
      it.createAsync({
        engine: this,
      })
    )
