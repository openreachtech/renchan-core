...
export default class BaseSubscriptionResolver extends BaseResolver {
  ...
  async subscribe ({
    ...
    const canSubscribe = this.canSubscribe({
      variables,
      context,
      information,
      parent,
   ...
   * }} params - Parameters.
   * @returns {boolean} - Whether can subscribe.
   */
  canSubscribe ({
    variables,
    context,
   ...
   *
   * @returns {import('../errors/RenchanGraphqlError.js').default} - CanNotSubscribe error.
   */
  createCanNotSubscribeError () {
    return this.Error.CanNotSubscribe
      .create()
  }
